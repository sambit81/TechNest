...
<title><%=p.getpTitle()%> | <%= user.getName() %></title>
...
<body>
					...
					<div class="card-body">
						...
						<div class="card-footer primary-background">
							...
							<a href="#!" onclick="doLike(<%=p.getPid()%>, <%=user.getId()%>)"
								...
								id="like-counter<%= p.getPid() %>"
								class="<%= ld.getClassNameForUser(p.getPid(), user.getId()) %>"></i><span
								class="like-counter<%= p.getPid() %>"> <%=ld.countLikeOnPost(p.getPid())%></span></a>
							<a href="#!" class="btn btn-outline-light btn-sm"><i
								class="fa-regular fa-comment"></i><span> <%= cDao.countCommentsOnPost(postId) %></span></a>
						</div>
						...
						<div class="card-footer">
							...
							<diV class="bg-light p-4">
							 ...
							 CommentDao cdao = new CommentDao(ConnectionProvider.getConnection()); 
							 %>
							    <% int cmts = cdao.countCommentsOnPost(p.getPid()); %>
								<span id="comment-counter-span" class="fs-5"><%= cmts %> <%= (cmts == 0 ? "comments" : cmts == 1 ? "comment" : "comments")  %></span>
								<div class="card p-3 mb-4 mt-3">
									<div class="d-flex align-items-center mb-2">
										<img src="img/<%= user.getProfile() %>" alt="User"
											class="profile-pic me-3"
											style="height: 50px; width: 50px; border-radius: 50%;">
									</div>
									<div class="card-body">
									<form id="cmt-form" method="post" action="CommentServlet">
										<div class="d-flex">
										<input type="text" name="pid" value="<%= p.getPid() %>" style="display: none;">
										<input type="text" name="uid" value="<%= user.getId() %>" style="display: none;">
										<input type="text" name="username" value="<%= user.getName() %>" style="display: none;">
										<textarea id="comment-box" class="form-control me-2 comment-box" rows="2" name="comment" required
											placeholder="Add a comment..."></textarea>
										<button type="submit" id="comment-btn" class="btn btn-primary">Comment</button>
									</div>
									</form>
									</div>
								</div>
					...
					<form id="add-post-form" action="AddPostServlet" method="post">
						<div class="form-group mb-2">
							<select class="form-control" name="cid" required>
								...
								<%
									...
									ArrayList<Category> clist = postd.getAllCategories();
						...
						<div class="form-group mb-2">
							<textarea name="pContent" class="form-control"
								style="height: 200px;" placeholder="Enter your content" required></textarea>
						</div>
  ...
  document.addEventListener("DOMContentLoaded", function () {
	...
	<script>
		$(document).ready(function(e) {
		$("#cmt-form").on("submit", function(event) {
			...
			$.ajax({
				...
				success : function(data, textStatus, jqXHR) {
					...
					if (data.trim() != 'error') {
						...
						fetchComments()
						$("#comment-counter-span").text(data)
						$("#cmt-form").trigger("reset")
					} else {
						Swal.fire({
							icon : "error",
							title : "Oops...",
